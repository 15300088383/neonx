...
func TestWrap(t *testing.T) {
	t.Run("err type not equal StackError", func(t *testing.T) {
		expectOutput := []string{
			...
			`github.com/geekymedic/neonx/errors.Wrap`,
		}
	...
	t.Run("err type equal StackError", func(t *testing.T) {
		expectOutput := []string{
			`github.com/geekymedic/neonx/errors.TestWrap.func2`,
		}
	...
	t.Run("err type equal StackError and mult call", func(t *testing.T) {
		expectOutput := []string{
			...
			`github.com/geekymedic/neonx/errors.stack_call_a`,
			`github.com/geekymedic/neonx/errors.stack_call_b`,
			`github.com/geekymedic/neonx/errors.stack_call_c`,
		}
		err := stack_call_c()
		for _, expect := range expectOutput {
			if !strings.Contains(err.Error(), expect) {
	...
	t.Run("err type equal StackError and mult call mannully", func(t *testing.T) {
		expectOutput := []string{
			`fail to open the file`,
			`github.com/geekymedic/neonx/errors.TestWrap.func4`,
			`github.com/geekymedic/neonx/errors/stack_error_test.go`,
		}
